...
var __globalBxslider = {
	...
	setup:function(idx, attr){
		if(attr.bxSlider){
			...
			_delay = _delay && !isNaN(parseInt(_delay)) ? parseInt(_delay) : 5000;
			...
			var _moves = $(attr).attr("data-moves");
			_moves = _moves && !isNaN(parseInt(_moves)) ? parseInt(_moves) : 1;

			var _loop = $(attr).attr("data-loop");
...
$(function() {
	...
	//탭클릭시 슬라이드 안되는 부분 수정 2016-12-02 영준주임
	/*$('[class*="js-tab-m"] a[href*="#"]').on('click', function(){
